...
class LoadingScreen:
    def __init__(self):
        ...
        self.ttlogo = OnscreenImage(image="phase_0/toontown-logo.png",scale=.45,pos=(0,0,.3))
        self.ttlogo.setSx(1)
        self.ttlogo.setSz(.8)
        ...
        for i in xrange(8): self.bar_anim_brpoints.append(randint(3,98))
...
class AvatarChoice(DirectButton):
    ...
    def __init__(self, position = 0, _ctpScr = None):
        ...
        if self.mode is AvatarChoice.MODE_CREATE:
            ...
            self['command'] = self.__handleChoice
            self['text'] = ('', L10N('CTP_PLAYTHIS'), L10N('CTP_PLAYTHIS'))
            self['text_scale'] = .12
            ...
            self.headModel = self._ctpScr.loadHead(self.head,self.position)
