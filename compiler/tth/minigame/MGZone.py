...
class MinigameZone(DirectObject):
    def __init__(self, zoneId, wantAvatar = True, wantCamera = False, wantControls = False, hud2keep = [0,1,1,1,0], environ = None):
        gamebase.curArea = self
        
        base.camera.setPos(0,0,0)
        base.camera.setHpr(0,0,0)
        self.avatar = gamebase.toonAvatar[0]._toon
        self.toon = gamebase.toonAvatar[0]
        self.toon.b_anim('neutral')
        ...
        self.task = taskMgr.add(self.area_task,zoneId)
        ...
        if wantCamera: base.camera.reparentTo(self.avatar)
        else: base.camera.reparentTo(render)
