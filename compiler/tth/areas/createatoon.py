...
class CreateAToon:
    ...
    def reloadToon(self):
        ...
        self.toon = EToon(self.np,self._hModels[self.data["spc"]],self.data["color1"],self.data["color2"],self.data["color3"],
    ...
    def loadZoneGui(self):
        ...
        if _zone == 0:
            ...
            self.gui_tnt = UnicodeText("Título",self.gui_tnb.getParent(),pos=(-1.085,0,.245),align = TextNode.ALeft, fg=Vec4(0,0,1,1), scale=.06)
            ...
            self.gui_lnt = UnicodeText("Último",self.gui_lnb.getParent(),pos=(-.078,0,.245),align = TextNode.ALeft, fg=Vec4(0,0,1,1), scale=.06)
            ...
            self.gui_fnt = UnicodeText("Primeiro",self.gui_fnb.getParent(),pos=(-.687,0,.245),align = TextNode.ALeft, fg=Vec4(0,0,1,1), scale=.06)
