...
class Area(PlaceBase):
            ...
            if target.name == fromTunnel:
                ...
                if not fromNp:
                    if 1:
                        ...
                        self.toon.b_setState('run') #('walk','run')
                    
                        self.toon._m.physControls.setCollisionsActive(False)
    ...
    def _restoreTunnelMovie(self,endPos = None, endH=0, endAvSc=None):
        self.toon._m.physControls.setCollisionsActive(True)
    ...
    def hideHud(self): aspect2d.hide()
    def showHud(self): aspect2d.show()
