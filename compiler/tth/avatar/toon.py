...
class HookedHead(ToonHead):
    def __init__(self,head,color,gender,gui=False):
        self.head = head
...
def loadToonHead(toontype,color1,color2,color3,numb,gender,gui=False,animMode = 1):
    ...
    print 'HEAD DNA HOOKER: CONVERTED:',toontype,numb,gender,'->',tt,_gender,gui
    
    head = HookedHead(tt,_color,_gender,gui)
...
class EToon:
    ...
    @staticmethod
    def makeDnaStatic(data):
        data["toontype"] = ("cat","pig","dog","horse","bear","mouse","duck","rabbit","monkey")[int(data['spc'])]
        data["color1"] = tuple(map(float,data["color1"].strip('()').split(',')))
        return data
