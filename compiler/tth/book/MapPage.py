...
class MapPage(BookPage):
    ...
    def setup(self):
        ...
        self.safezones = [2000,1000,5000,4000,3000,2400,1400,6000]
        ...
        zone = int(gamebase.toonAvatarStream.read("lastZoneId","0"))
        ...
        if False: #base.cr.playGame.getPlaceId() == ToontownGlobals.MyEstate:
            ...
            streetName = L10N('AREA_ST_'+str(gamebase.toonAvatar[0].zoneId%10**7))
    ...
    def __buttonCallback(self, hood):
        if hood in self.hoodTeleportList:
            print 'Teleport',hood
            self.book.close()
            gamebase.curArea.handleAnim = False
            gamebase.curArea.disableControls()
            gamebase.curArea.toon.b_setState('Teleport',str(hood))
