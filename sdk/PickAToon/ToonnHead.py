...
class ToonHead(Actor.Actor):
    ...
    def __init__(self):
        try:
            ...
            self.EyesOpen = loader.loadTexture('phase_3/maps/eyes.jpg', 'phase_3/maps/eyes_a.rgb')
            self.EyesOpen.setMinfilter(Texture.FTLinear)
            self.EyesOpen.setMagfilter(Texture.FTLinear)
            self.EyesClosed = loader.loadTexture('phase_3/maps/eyesClosed.jpg', 'phase_3/maps/eyesClosed_a.rgb')
            self.EyesClosed.setMinfilter(Texture.FTLinear)
            self.EyesClosed.setMagfilter(Texture.FTLinear)
            self.EyesSadOpen = loader.loadTexture('phase_3/maps/eyesSad.jpg', 'phase_3/maps/eyesSad_a.rgb')
            self.EyesSadOpen.setMinfilter(Texture.FTLinear)
            self.EyesSadOpen.setMagfilter(Texture.FTLinear)
            self.EyesSadClosed = loader.loadTexture('phase_3/maps/eyesSadClosed.jpg', 'phase_3/maps/eyesSadClosed_a.rgb')
            self.EyesSadClosed.setMinfilter(Texture.FTLinear)
            self.EyesSadClosed.setMagfilter(Texture.FTLinear)
            self.EyesAngryOpen = loader.loadTexture('phase_3/maps/eyesAngry.jpg', 'phase_3/maps/eyesAngry_a.rgb')
            self.EyesAngryOpen.setMinfilter(Texture.FTLinear)
            self.EyesAngryOpen.setMagfilter(Texture.FTLinear)
            self.EyesAngryClosed = loader.loadTexture('phase_3/maps/eyesAngryClosed.jpg', 'phase_3/maps/eyesAngryClosed_a.rgb')
            self.EyesAngryClosed.setMinfilter(Texture.FTLinear)
            self.EyesAngryClosed.setMagfilter(Texture.FTLinear)
            self.EyesSurprised = loader.loadTexture('phase_3/maps/eyesSurprised.jpg', 'phase_3/maps/eyesSurprised_a.rgb')
            self.EyesSurprised.setMinfilter(Texture.FTLinear)
            self.EyesSurprised.setMagfilter(Texture.FTLinear)
            self.Muzzle = loader.loadTexture('phase_3/maps/muzzleShrtGeneric.jpg')
            self.Muzzle.setMinfilter(Texture.FTLinear)
            self.Muzzle.setMagfilter(Texture.FTLinear)
            self.MuzzleSurprised = loader.loadTexture('phase_3/maps/muzzleShortSurprised.jpg')
            self.MuzzleSurprised.setMinfilter(Texture.FTLinear)
            self.MuzzleSurprised.setMagfilter(Texture.FTLinear)
    ...
    def setupHead(self, dna, forGui = 0):
        ...
        bodyScale = {'horse': 0.85, 'monkey': 0.69, 'dog': 0.85, 'bear': 0.85,
                     'cat': 0.73, 'rabbit': 0.74, 'duck': 0.67, 'pig': 0.78,
                     'mouse': 0.6}[animalStyle] 
        headScale = ((1,1,1),(.75,1,1))[animalStyle=='mouse'] 
    ...
    def sadEyes(self):
        self.__eyesOpen = self.EyesSadOpen
        self.__eyesClosed = self.EyesSadClosed

    def angryEyes(self):
        self.__eyesOpen = self.EyesAngryOpen
        self.__eyesClosed = self.EyesAngryClosed
    ...
    def enterEyelidsSurprised(self):
        if not self.__eyes.isEmpty() and self.EyesSurprised:
            self.__eyes.setTexture(self.EyesSurprised, 1)
            if self.__eyelashOpen:
                self.__eyelashOpen.hide()
            if self.__eyelashClosed:
                self.__eyelashClosed.hide()
            if self.__lod500Eyes:
                self.__lod500Eyes.setTexture(self.EyesSurprised, 1)
            if self.__lod250Eyes:
                self.__lod250Eyes.setTexture(self.EyesSurprised, 1)
            if self.__muzzle:
